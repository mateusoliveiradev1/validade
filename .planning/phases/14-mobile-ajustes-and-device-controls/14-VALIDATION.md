...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 14-01-01 | 01 | 1 | SET-01 | T-14-01 | Ajustes opens without replacing session/store/role truth | component/unit | `cmd /c pnpm.cmd --filter @validade-zero/mobile test` | yes | pending |
| 14-01-02 | 01 | 1 | SET-01 | T-14-02 | Route stack returns to the exact previous operational route | component/unit | `cmd /c pnpm.cmd --filter @validade-zero/mobile test` | yes | pending |
| 14-02-01 | 02 | 2 | SET-02 | T-14-03 | Push activation/test remains diagnostic and this-device scoped | component/unit | `cmd /c pnpm.cmd --filter @validade-zero/mobile test` | yes | pending |
| 14-02-02 | 02 | 2 | SET-02 | T-14-04 | Disable path affects only this device and leaves tasks active in Hoje | component/unit | `cmd /c pnpm.cmd --filter @validade-zero/mobile test` | yes | pending |
| 14-03-01 | 03 | 3 | SET-03 | T-14-05 | Sync card separates central read from sync send state | component/unit | `cmd /c pnpm.cmd --filter @validade-zero/mobile test` | yes | pending |
| 14-03-02 | 03 | 3 | SET-03 | T-14-06 | Conflict discard requires explicit reason and blocks safe close when critical | component/unit | `cmd /c pnpm.cmd --filter @validade-zero/mobile test` | yes | pending |
| 14-04-01 | 04 | 4 | SET-04 | T-14-07 | Build/update card shows public-safe installed versus approved APK truth | component/unit | `cmd /c pnpm.cmd --filter @validade-zero/mobile test` | yes | pending |
| 14-04-02 | 04 | 4 | SET-05 | T-14-08 | Privacy and sign-out use existing auth model and warn on pending work | component/unit | `cmd /c pnpm.cmd --filter @validade-zero/mobile test` | yes | pending |
| 14-05-01 | 05 | 5 | SET-01..SET-05 | T-14-09 | Mobile release journey proves Ajustes boundaries and no sensitive text | journey/unit | `cmd /c pnpm.cmd --filter @validade-zero/mobile test` | yes | pending |
| 14-05-02 | 05 | 5 | SET-01..SET-05 | T-14-SC | Full repo gates pass without Android/provider/physical-proof overclaim | repo gate | `cmd /c pnpm.cmd check` | yes | pending |
