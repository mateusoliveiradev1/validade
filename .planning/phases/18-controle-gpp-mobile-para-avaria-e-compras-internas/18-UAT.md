...
updated: 2026-07-10T00:11:00-03:00
---
...
result: issue
reported: "A central rejection becomes visible as `Conflito de GPP` and discard is disabled without justification, but after a justification is entered the enabled `Descartar registro deste aparelho` button does nothing."
severity: major
...
- truth: "A central rejection becomes visible as `Conflito de GPP`; discard requires a local justification and then removes the record from the active local queue."
  status: failed
  reason: "Autonomous emulator UAT: rejection and required justification work, but the enabled discard action does nothing after the reason is entered."
  severity: major
  test: 5
  ...
  missing:
    - "Wire a CaptureApp discard handler to repository.discardGppPending with the supplied justification and refresh local GPP lists."
    - "Add an integration test that enters a discard reason, presses the enabled action, and verifies the conflict leaves the active queue."
