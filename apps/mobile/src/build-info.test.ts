...
describe("mobile build info", () => {
  ...
  it("falls back to Expo config when native metadata is unavailable", () => {
    const info = readMobileBuildInfo({
      ...
      constants: {
        expoConfig: {
          version: "0.12.0",
          android: { package: "com.validadezero.app", versionCode: 158 },
          extra: {},
        },
      },
    });
  ...
  it("marks old, future, and unknown builds without leaking raw build URLs", () => {
    ...
    expect(unknownBuild.approvedArtifactLabel).toBe("uat28-cloud-observacao-processado-apk-158");
    expect(unknownBuild.approvedBuild).toBe("158");
