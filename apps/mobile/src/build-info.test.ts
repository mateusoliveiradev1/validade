...
describe("mobile build info", () => {
  ...
  it("falls back to Expo config when native metadata is unavailable", () => {
    const info = readMobileBuildInfo({
      ...
      constants: {
        expoConfig: {
          version: "0.12.0",
          android: { package: "com.validadezero.app", versionCode: 150 },
          extra: {},
        },
      },
    });
  ...
  it("marks old, future, and unknown builds without leaking raw build URLs", () => {
    ...
    expect(unknownBuild.approvedArtifactLabel).toBe("uat20-onboarding-shift-e2e-apk-150");
    expect(unknownBuild.approvedBuild).toBe("150");
