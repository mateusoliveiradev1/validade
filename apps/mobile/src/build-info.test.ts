...
describe("mobile build info", () => {
        ...
        default: {
          expoConfig: {
            ...
            extra: {
              ...
              VALIDADE_ZERO_APPROVED_ARTIFACT_LABEL: "uat15-sync-debug-apk-136",
              VALIDADE_ZERO_APPROVED_APP_VERSION: "0.12.0",
              VALIDADE_ZERO_APPROVED_BUILD: "136",
  ...
  it("falls back to Expo config when native metadata is unavailable", () => {
    const info = readMobileBuildInfo({
      ...
      constants: {
        expoConfig: {
          version: "0.12.0",
          android: { package: "com.validadezero.app", versionCode: 136 },
          extra: {},
        },
      },
    });
  ...
  it("marks old, future, and unknown builds without leaking raw build URLs", () => {
    ...
    expect(unknownBuild.approvedArtifactLabel).toBe("uat15-sync-debug-apk-136");
    expect(unknownBuild.approvedBuild).toBe("136");
