  ...
  it("replays locally kept central observations automatically after a refreshed central read", async () => {
    ...
    const appendCentralObservation = vi.fn((centralLotId, request) => {
      ...
      return Promise.resolve({
        requestId: "append-observation-central-recentes-replay-001",
        lot: {
          centralLotId,
          centralProductId: "produto-central-recentes-001",
          productDisplayName: "Mamao Recentes FICTICIO",
          ...
          initialLocation: { kind: "area_de_venda" as const },
          currentObservation: {
            centralObservationId: "observacao-central-recentes-replay-perda-001",
            centralLotId,
            ...request.observation,
          },
          lifecycleStatus: "resolved" as const,
        ...
        acknowledgement: {
          acknowledgementId: "ack-observacao-central-recentes-replay-perda-001",
          centralLotId,
          state: "resolved" as const,
          acknowledgedAt: "2030-01-10T09:12:00.000Z",
          message: "Observacao salva na central.",
    ...
    const repository = createMemoryCaptureRepository({
      clock: () => "2030-01-10T09:00:00.000Z",
      createId: () => "observacao-local-recentes-replay-perda-001",
      appendCentralObservation,
    });
    ...
    const localObservation = await repository.appendObservation("lote-central-recentes-001", {
      status: "loss",
      actorLabel: "Colaboradora FICTICIA",
      occurredAt: "2030-01-10T09:12:00.000Z",
      location: { kind: "area_de_venda" },
    ...
    expect(localObservation).toMatchObject({
      id: "observacao-local-recentes-replay-perda-001",
      status: "loss",
      location: { kind: "retirada_perda" },
    });
    await expect(repository.listSyncQueue()).resolves.toMatchObject({
    ...
    await expect(repository.syncPendingCentralLots?.()).resolves.toEqual([
      expect.objectContaining({
        ...
        currentObservation: expect.objectContaining({
          id: "observacao-central-recentes-replay-perda-001",
          status: "loss",
          location: { kind: "retirada_perda" },
        }),
      }),
    ]);
    expect(appendCentralObservation).toHaveBeenCalledTimes(2);
    expect(appendCentralObservation).toHaveBeenLastCalledWith(
      "lote-central-recentes-001",
      expect.objectContaining({
        observation: expect.objectContaining({
          status: "loss",
