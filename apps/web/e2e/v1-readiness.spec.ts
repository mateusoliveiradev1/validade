...
test("operational readiness routes keep each truth in its own room", async ({ page }) => {
  ...
  expect(pageText).not.toContain("uat27-vencimento-detalhe-apk-157");
  ...
  await expect(page.getByText("uat27-vencimento-detalhe-apk-157")).toBeVisible();
  ...
  await expect(page.getByText("uat27-vencimento-detalhe-apk-157")).toHaveCount(0);
  await expect(page.getByText("Enviar teste seguro")).toHaveCount(0);
  await expect(page.getByText("Ver instrucoes manuais")).toHaveCount(0);
