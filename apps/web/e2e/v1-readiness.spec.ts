...
test("operational readiness routes keep each truth in its own room", async ({ page }) => {
  ...
  expect(pageText).not.toContain("uat18-guided-onboarding-apk-148");
  ...
  await expect(page.getByText("uat18-guided-onboarding-apk-148")).toBeVisible();
  ...
  await expect(page.getByText("uat18-guided-onboarding-apk-148")).toHaveCount(0);
  await expect(page.getByText("Enviar teste seguro")).toHaveCount(0);
  await expect(page.getByText("Ver instrucoes manuais")).toHaveCount(0);
