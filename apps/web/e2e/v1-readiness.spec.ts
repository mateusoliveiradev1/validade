...
test("operational readiness routes keep each truth in its own room", async ({ page }) => {
  ...
  expect(pageText).not.toContain("uat21-overdue-task-fix-apk-151");
  ...
  await expect(page.getByText("uat21-overdue-task-fix-apk-151")).toBeVisible();
  ...
  await expect(page.getByText("uat21-overdue-task-fix-apk-151")).toHaveCount(0);
  await expect(page.getByText("Enviar teste seguro")).toHaveCount(0);
  await expect(page.getByText("Ver instrucoes manuais")).toHaveCount(0);
