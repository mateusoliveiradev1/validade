...
test("operational readiness routes keep each truth in its own room", async ({ page }) => {
  ...
  expect(pageText).not.toContain("uat20-onboarding-shift-e2e-apk-150");
  ...
  await expect(page.getByText("uat20-onboarding-shift-e2e-apk-150")).toBeVisible();
  ...
  await expect(page.getByText("uat20-onboarding-shift-e2e-apk-150")).toHaveCount(0);
  await expect(page.getByText("Enviar teste seguro")).toHaveCount(0);
  await expect(page.getByText("Ver instrucoes manuais")).toHaveCount(0);
