...
test("operational readiness routes keep each truth in its own room", async ({ page }) => {
  ...
  expect(pageText).not.toContain("uat19-first-turn-onboarding-apk-149");
  ...
  await expect(page.getByText("uat19-first-turn-onboarding-apk-149")).toBeVisible();
  ...
  await expect(page.getByText("uat19-first-turn-onboarding-apk-149")).toHaveCount(0);
  await expect(page.getByText("Enviar teste seguro")).toHaveCount(0);
  await expect(page.getByText("Ver instrucoes manuais")).toHaveCount(0);
