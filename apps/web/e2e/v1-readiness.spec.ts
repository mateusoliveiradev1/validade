...
test("operational readiness routes keep each truth in its own room", async ({ page }) => {
  ...
  expect(pageText).not.toContain("uat24-recentes-resolved-apk-154");
  ...
  await expect(page.getByText("uat24-recentes-resolved-apk-154")).toBeVisible();
  ...
  await expect(page.getByText("uat24-recentes-resolved-apk-154")).toHaveCount(0);
  await expect(page.getByText("Enviar teste seguro")).toHaveCount(0);
  await expect(page.getByText("Ver instrucoes manuais")).toHaveCount(0);
