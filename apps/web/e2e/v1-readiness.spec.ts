...
test("operational readiness routes keep each truth in its own room", async ({ page }) => {
  ...
  expect(pageText).not.toContain("uat23-sync-orphan-cleanup-apk-153");
  ...
  await expect(page.getByText("uat23-sync-orphan-cleanup-apk-153")).toBeVisible();
  ...
  await expect(page.getByText("uat23-sync-orphan-cleanup-apk-153")).toHaveCount(0);
  await expect(page.getByText("Enviar teste seguro")).toHaveCount(0);
  await expect(page.getByText("Ver instrucoes manuais")).toHaveCount(0);
